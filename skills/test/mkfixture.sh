...
if $CHAINED_PRE_PUSH; then
	...
	cat >"$ROOT/.git/hooks/pre-push.pre-entire" <<'EOF'
#!/bin/sh
...
# $FOREIGN_MARKER
_d="\$(dirname "\$0")"
[ -x "\$_d/pre-push.pre-entire" ] && "\$_d/pre-push.pre-entire" "\$@"
EOF
	chmod +x "$ROOT/.git/hooks/pre-push" "$ROOT/.git/hooks/pre-push.pre-entire"
fi
